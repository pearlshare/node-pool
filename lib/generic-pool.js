...
Pool.prototype._dispense = function dispense () {
  ...
  if (waitingCount > 0) {
    ...
    while (this._availableObjects.length > 0) {
      ...
      if (process.env.PSKNEXDEBUG === 'true') {
        console.log('PSKNEXDEBUG - Verifying with validator:' + (this._factory.validate || '').toString())
      }
      if (this._factory.validate && !this._factory.validate(objWithTimeout.obj)) {
